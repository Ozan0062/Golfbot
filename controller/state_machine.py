...
class GolfBotController:
    ...
    def _seek(self, pose, world) -> Command:
        ...
        # 3. Ball against a wall or in a corner?  Approach via staging points.
    ...
    def _cross_ball_approach(self, pose, target, world) -> bool:
        ...
        radius = cross_trigger_radius(
            world.get("cross_size_px"), CROSS_RADIUS_PX, CROSS_BALL_CLEARANCE_PX)
    ...
    def _stage_along_angle(self, pose, target, approach_angle, label) -> State:
        ...
        if not waypoints:                       # already at the staging point — go straight in
            log.debug("Already at %s staging point — heading straight in", label)
            return State.APPROACH
